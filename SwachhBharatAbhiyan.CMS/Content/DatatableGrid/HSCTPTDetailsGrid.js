...
function loadGridCTPT() {
    ...
    $("#demoGrid4").DataTable({
        buttons: [
            ...
            {
                extend: 'excel', className: 'btn btn-sm btn-success filter-button-style', title: appName + ' CTPT Report', text: 'Export to Excel', exportOptions: { columns: [0, 1, 2, 3, 4, 5] }
            },
        ],
        //"sDom": "ltipr",
        dom: 'lBfrtip',
            ...
            },
            {
                ...
                "render": function (data, type, full, meta) {
                    if (full["QRCodeImage"] != null) {
                        return "<div style='cursor:pointer;display:inline-flex;'  onclick=PopImagesCTPT(this)><img alt='Photo Not Found'  src='" + data +
    ...
    var tableCTPT = $('#demoGrid4').DataTable();
    $("#target2").click(function () {
        //alert("Handler for .click() called.");
        var tableCTPT = $('#demoGrid4').DataTable();
        tableCTPT.ajax.reload(null, false);
    });
    $('#demoGrid4').on('order.dt', function () {
        ...
        searchString = $("#sCTPT").val();
        // This will show: "Ordering on column 1 (asc)", for example
        var order = tableCTPT.order();
        //console.log(order);
        if (order.length > 0) {
            var sortColumn = tableCTPT.settings().init().columns[order[0][0]].name;
        ...
        $.ajax({
            type: "GET",
            url: "/HouseScanify/GetHSCTPTDetailsID?fdate=" + txt_fdate + "&tdate=" + txt_tdate + "&userId=" + UserId + "&searchString=" + searchString + "&qrStatus=" + QRStatus + "&sortColumn=" + sortColumn + "&sortOrder=" + sortOrder,
            datatype: "json",
            traditional: true,
            success: function (data) {
                arrCTPTIDs = data;
                console.log(arrCTPTIDs);
...
function SearchCTPT() {
    ...
    var Product = "";
    var catProduct = "";
    var value = txt_fdate + "," + txt_tdate + "," + UserId + "," + $("#sCTPT").val() + "," + QRStatus;//txt_fdate + "," + txt_tdate + "," + UserId + "," + Client + "," + NesEvent + "," + Product + "," + catProduct + "," + 1;
    // alert(value );
    oTable = $('#demoGrid4').DataTable();
    oTable.search(value).draw();
    oTable.search("");
