...
$("#demoGrid3").dataTable().fnDestroy();
        ...
        },
        {
            ...
            "render": function (data, type, full, meta) {
                if (full["QRCodeImage"] != null) {
                    return "<div style='cursor:pointer;display:inline-flex;'  onclick=PopImagesSW(this)><img alt='Photo Not Found'  src='" + data +
    ...
    var tableSW = $('#demoGrid3').DataTable();
    $("#target5").click(function () {
        //alert("Handler for .click() called.");
        var tableSW = $('#demoGrid3').DataTable();
        tableSW.ajax.reload(null, false);
    });
    $('#demoGrid3').on('order.dt', function () {
        ...
        var order = tableSW.order();
        //console.log(order);
        if (order.length > 0) {
            var sortColumn = tableSW.settings().init().columns[order[0][0]].name;
        ...
        $.ajax({
            type: "GET",
            url: "/HouseScanify/GetHSSWDetailsID?fdate=" + txt_fdate + "&tdate=" + txt_tdate + "&userId=" + UserId + "&searchString=" + searchString + "&qrStatus=" + QRStatus + "&sortColumn=" + sortColumn + "&sortOrder=" + sortOrder,
            datatype: "json",
            traditional: true,
            success: function (data) {
                arrSWIDs = data;
                console.log(arrSWIDs);
