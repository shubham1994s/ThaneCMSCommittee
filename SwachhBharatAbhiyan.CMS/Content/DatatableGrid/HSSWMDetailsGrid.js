...
function loadGridSWM() {
    ...
    $("#demoGrid5").DataTable({
        ...
        "ajax": {
            "url": "/Datable/GetJqGridJson?rn=HSSWMDetails",
            "type": "POST",
            "datatype": "json"
        },
            ...
            },
            {
                ...
                "render": function (data, type, full, meta) {
                    if (full["QRCodeImage"] != null) {
                        return "<div style='cursor:pointer;display:inline-flex;'  onclick=PopImagesSWM(this)><img alt='Photo Not Found'  src='" + data +
    ...
    var tableSWM = $('#demoGrid5').DataTable();
    $("#target3").click(function () {
        //alert("Handler for .click() called.");
        var tableSWM = $('#demoGrid5').DataTable();
        tableSWM.ajax.reload(null, false);
    });
    $('#demoGrid5').on('order.dt', function () {
        ...
        searchString = $("#sSWM").val();
        // This will show: "Ordering on column 1 (asc)", for example
        var order = tableSWM.order();
        //console.log(order);
        if (order.length > 0) {
            var sortColumn = tableSWM.settings().init().columns[order[0][0]].name;
        ...
        $.ajax({
            type: "GET",
            url: "/HouseScanify/GetHSSWMDetailsID?fdate=" + txt_fdate + "&tdate=" + txt_tdate + "&userId=" + UserId + "&searchString=" + searchString + "&qrStatus=" + QRStatus + "&sortColumn=" + sortColumn + "&sortOrder=" + sortOrder,
            datatype: "json",
            traditional: true,
            success: function (data) {
                arrSWMIDs = data;
                console.log(arrSWMIDs);
...
function SearchSWM() {
    ...
    var Product = "";
    var catProduct = "";
    var value = txt_fdate + "," + txt_tdate + "," + UserId + "," + $("#sSWM").val() + "," + QRStatus;//txt_fdate + "," + txt_tdate + "," + UserId + "," + Client + "," + NesEvent + "," + Product + "," + catProduct + "," + 1;
    // alert(value );
    oTable = $('#demoGrid5').DataTable();
    oTable.search(value).draw();
    oTable.search("");
