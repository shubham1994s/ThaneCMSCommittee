...
function loadGridCommercial() {
    ...
    $("#demoGrid1").DataTable({
        buttons: [
            ...
            {
                extend: 'excel', className: 'btn btn-sm btn-success filter-button-style', title: appName + ' Commercial Report', text: 'Export to Excel', exportOptions: { columns: [0, 1, 2, 3, 4, 5] }
            },
        ],
        //"sDom": "ltipr",
        dom: 'lBfrtip',
        ...
        "ajax": {
            "url": "/Datable/GetJqGridJson?rn=HSCommercialDetails",
            "type": "POST",
            "datatype": "json"
        },
            ...
            },
            {
                ...
                "render": function (data, type, full, meta) {
                    if (full["QRCodeImage"] != null) {
                        return "<div style='cursor:pointer;display:inline-flex;'  onclick=PopImagesComr(this)><img alt='Photo Not Found'  src='" + data +
    ...
    var tableComr = $('#demoGrid1').DataTable();
    $("#target1").click(function () {
        //alert("Handler for .click() called.");
        var tableComr = $('#demoGrid1').DataTable();
        tableComr.ajax.reload(null, false);
    });
    $('#demoGrid1').on('order.dt', function () {
        ...
        searchString = $("#sComer").val();
        // This will show: "Ordering on column 1 (asc)", for example
        var order = tableComr.order();
        //console.log(order);
        if (order.length > 0) {
            var sortColumn = tableComr.settings().init().columns[order[0][0]].name;
        ...
        $.ajax({
            type: "GET",
            url: "/HouseScanify/GetHSComrDetailsID?fdate=" + txt_fdate + "&tdate=" + txt_tdate + "&userId=" + UserId + "&searchString=" + searchString + "&qrStatus=" + QRStatus + "&sortColumn=" + sortColumn + "&sortOrder=" + sortOrder,
            datatype: "json",
            traditional: true,
            success: function (data) {
                arrComrIDs = data;
                console.log(arrComrIDs);
...
function SearchCommercial() {
    ...
    var Product = "";
    var catProduct = "";
    var value = txt_fdate + "," + txt_tdate + "," + UserId + "," + $("#sComer").val() + "," + QRStatus;//txt_fdate + "," + txt_tdate + "," + UserId + "," + Client + "," + NesEvent + "," + Product + "," + catProduct + "," + 1;
    // alert(value );
    oTable = $('#demoGrid1').DataTable();
    oTable.search(value).draw();
    oTable.search("");
