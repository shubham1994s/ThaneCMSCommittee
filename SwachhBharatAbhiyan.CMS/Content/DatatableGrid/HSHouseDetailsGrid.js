...
function loadGridHouse() {
    ...
    $("#demoGrid").DataTable({
        ...
        "columnDefs":
            [{
                ...
                { bSortable: false, targets: [4, 5, 6, 7, 8, 9] },
                {
                    "targets": [10],
                    "visible": false,
                    "searchable": false
    ...
    $('#demoGrid').on('order.dt', function () {
        ...
        $.ajax({
            type: "GET",
            url: "/HouseScanify/GetHSHouseDetailsID?fdate=" + txt_fdate + "&tdate=" + txt_tdate + "&userId=" + UserId + "&searchString=" + searchString + "&qrStatus=" + QRStatus + "&sortColumn=" + sortColumn + "&sortOrder=" + sortOrder,
            datatype: "json",
            traditional: true,
