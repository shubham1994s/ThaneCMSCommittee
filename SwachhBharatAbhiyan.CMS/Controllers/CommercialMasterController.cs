...
{
    public class CommercialMasterController : Controller
    {
        // GET: CommercialMaster
        IChildRepository childRepository;
        IMainRepository mainRepository;

        public CommercialMasterController()
        {
        ...
        [HttpPost]
        public ActionResult AddHouseDetails(CommercialDetailsVM house)
        {
            if (SessionHandler.Current.AppId != 0)
            {
                ...
                if (houseId.houseQRCode == null)
                {
                    ...
                    string imgpath = Path.Combine(Server.MapPath(AppDetails.basePath + AppDetails.CommercialQRCode), image_Guid);
                    var exists = System.IO.Directory.Exists(Server.MapPath(AppDetails.basePath + AppDetails.CommercialQRCode));
                    if (!exists)
                    {
                        System.IO.Directory.CreateDirectory(Server.MapPath(AppDetails.basePath + AppDetails.CommercialQRCode));
                    }
                    img.Save(imgpath);
                    response.Close();
                    remoteStream.Close();
